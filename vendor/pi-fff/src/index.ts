...
import { orderingFromEnv } from "../../../src/rank.ts";
import type {
  ExtensionAPI,
  ExtensionContext,
  ToolDefinition,
...
export default function fffExtension(pi: ExtensionAPI) {
  ...
  const grepSchema = Type.Object({
    intent: Type.Optional(Type.String({
      description: "Required for fresh searches. Describe the implementation or evidence you need. May be omitted when continuing a cursor.",
    })),
    pattern: Type.String({
    ...
    context: Type.Optional(
      Type.Number({
        description: `Context lines before+after each match (0-${GREP_CONTEXT_MAX}, default 3)`,
      }),
    ),
    limit: Type.Optional(
      Type.Number({
  ...
  queueTool(() => toolNames.grep, {
    description: `Grep file contents. Supply a literal/regex pattern plus intent describing the evidence you need. Smart-case, git-aware. A bounded candidate pool is ordered for relevance; each snippet preserves source line order. This is not a whole-repository ranking. Default limit ${DEFAULT_GREP_LIMIT}. Use the cursor to see remaining candidates.`,
    promptSnippet: "Grep contents",
    promptGuidelines: (names) => [
      `${names.grep}: prefer bare identifiers as patterns. Literal queries are most efficient. Include intent on each fresh search.`,
    ...
    async execute(_toolCallId, params, signal) {
      ...
      const searchCwd = activeCwd;
      const context = clampContext(params.context ?? 3);
      const signature = JSON.stringify([searchCwd, pattern, params.path ?? null, params.exclude ?? null, params.caseSensitive ?? false, context]);
      ...
      let fuzzyQuery: string | undefined;
      return grepPilot.search({
        signature, intent: params.intent, order: orderingFromEnv(),
        cursor: params.cursor, limit: params.limit, signal,
        scan: async (cursor) => {
          // Reacquire auxiliary finders because idle pools can be evicted between pages.
          const aux = await resolveFinderForPath(params.path, pattern, params.exclude);
          const picker = aux ? aux.finder : await ensureFinder(searchCwd);
          const query = aux ? aux.query : buildQuery(params.path, pattern, params.exclude, searchCwd);
          const options = {
            mode: fuzzyQuery === undefined ? mode : "fuzzy" as GrepMode,
            smartCase, maxMatchesPerFile: GREP_MAX_MATCHES_PER_FILE,
            pageSize: GREP_PAGE_SIZE_MAX, cursor,
            beforeContext: fuzzyQuery === undefined ? context : 0,
            afterContext: fuzzyQuery === undefined ? context : 0,
            classifyDefinitions: true, timeBudgetMs: GREP_TIME_BUDGET_MS,
          };
          const grepResult = picker.grep(fuzzyQuery ?? query, options);
          if (!grepResult.ok) throw new Error(grepResult.error);
          let result = grepResult.value;
          if (!cursor && fuzzyQuery === undefined && result.items.length === 0 && !result.nextCursor && mode !== "regex") {
            const lastSeg = params.path?.split(/[\\/]/).pop() ?? "";
            const pathTargetsFile = /\.[a-zA-Z][a-zA-Z0-9]{0,9}$/.test(lastSeg);
            const broadenedQuery = pathTargetsFile ? pattern : query;
            const fuzzy = picker.grep(broadenedQuery, { ...options, mode: "fuzzy", beforeContext: 0, afterContext: 0 });
            if (fuzzy.ok && fuzzy.value.items.length > 0) {
              fuzzyQuery = broadenedQuery;
              result = fuzzy.value;
            }
          }
          if (aux) result = { ...result, items: result.items.map((item) => ({ ...item, relativePath: nodePath.resolve(aux.root, item.relativePath) })) };
          return { result, notice: fuzzyQuery === undefined ? undefined : "0 exact matches. Fuzzy suggestions follow; a filename constraint may have been broadened." };
        },
      });
    },
